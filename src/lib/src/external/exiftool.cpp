...
bool Exiftool::setMetadata(const QString &file, const QMap<QString, QString> &metadata, bool clear, bool keepColorProfile, int msecs)
{
	...
	if (clear) {
		commands.append("-all=");
		if (keepColorProfile) {
			commands.append({ "--icc_profile:all", "-tagsfromfile", "@", "-colorspacetags" });
		}
	}
