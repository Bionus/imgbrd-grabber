...
TEST_CASE("TagStylist")
{
	...
	SECTION("KeptForLater")
	{
		...
		settings.setValue("Coloring/Colors/keptForLater", "#ffff7f");
		...
		QString actual = stylist.stylished(QList<Tag>() << tag).join("");
		std::string expected = "<a href=\"tag_text\" style=\"color:#ffff7f; font-family:'[^']*'; font-size:[0-9]+pt; font-style:normal; font-weight:400; text-decoration:none;\">tag_text</a>";
		REQUIRE_THAT(actual.toStdString(), Matches(expected));
	}
