            ...
            search: {
                ...
                parse: (src: string, statusCode: number): IParsedSearch | IError => {
                    ...
                    return {
                        ...
                        images: Grabber.regexToImages(`<a href=['"][^'">]*/post/view/(?<id>[^'"]+)['"][^>]*>[^<]*(?<image><img(?: id=['"](?:[^'"]*)['"]) title=['"](?<tags>[^'"\/]*)(?:(?: \/\/ |\n)(?<width>[^'"\/]+?)x(?<height>[^'"\/]+?) \/\/ (?<file_size>[^'"\/]*?)(?: \/\/ (?<ext>[^'"\/]*?))?)?(?:\s+[^'"\/]*)?['"] alt=['"][^'"]*['"] height=['"](?<preview_height>[^'"]*)['"] width=['"](?<preview_width>[^'"]*)['"][^>]*(?:src|data-original)=['"](?<preview_url>[^'"]*(?:\/_thumbs\/(?<md5>[^\/]*)\/[^'"]*\.(?:jpg|webp)|\/thumb\/(?<md5_2>[^'"]*)\.(?:jpg|webp)|questionable\.jpg|\/thumbs\/[^\/]{2}\/(?<md5_5>[^'"]+)(?:\.(?:jpg|webp))?))['"][^>]*>).*?<\/a>(?:<br\s*\/?><a href=['"](?<file_url>[^'"]+)['"][^>]*>File Only)?`, src).map(completeImage),
                        pageCount,
                    };
                },
            },
