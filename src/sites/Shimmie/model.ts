            ...
            search: {
                ...
                parse: (src: string, statusCode: number): IParsedSearch | IError => {
                    ...
                    return {
                        ...
                        images: Grabber.regexToImages("<a(?: class=['\"][^'\"]*['\"])? href=['\"][^'\">]*/post/view/(?<id>[^'\"]+)['\"][^>]*>(?:<div[^>]*>[^<]*</div>)?[^<]*(?<image><img(?: id=['\"](?:[^'\"]*)['\"])? title=['\"](?<tags>[^'\"/]*)(?:(?: // |\\n)(?<width>[^'\"/]+?)x(?<height>[^'\"/]+?) // (?<file_size>[^'\"/]*?)(?: // (?<ext>[^'\"/]*?))?)?(?:\\s+[^'\"/]*)?['\"] alt=['\"][^'\"]*['\"](?: height=['\"](?<preview_height>[^'\"]*)['\"])? width=['\"](?<preview_width>[^'\"]*)['\"](?: height=['\"](?<preview_height_2>[^'\"]*)['\"])?[^>]*(?:src|data-original)=['\"](?<preview_url>[^'\"]*(?:/_thumbs/(?<md5>[^/]*)/[^'\"]*\\.(?:jpg|webp)|/thumb/(?<md5_2>[^'\"]*)\\.jpg|questionable\\.jpg|/thumbs/[^/]{2}/(?<md5_5>[^'\"]+)(?:\\.jpg)?))['\"][^>]*>).*?</a>(?:<br\\s*/?><a href=['\"](?<file_url>[^'\"]+)['\"][^>]*>File Only)?|<a href=['\"][^'\">]*/i(?<id_2>[^'\"]+)['\"](?: class=['\"][^'\"]*['\"])?[^>]*>[^<]*(?<image_2><img(?: id=['\"](?:[^'\"]*)['\"])? src=['\"](?<preview_url_2>[^'\"]*(?:/_thumbs/(?<md5_3>[^'\"]*)(?:_th)?\\.jpg|/thumb/(?<md5_4>[^'\"]*)\\.jpg|questionable\\.jpg|/thumbs/[^/]{2}/(?<md5_6>[^'\"]+)(?:\\.jpg)?))['\"] title=['\"](?<tags_2>[^'\"/]+) // (?<width_2>[^'\"]+)x(?<height_2>[^'\"]+) // (?<file_size_2>[^'\"]*)(?: // (?<ext_2>[^'\"]*))?['\"] alt=['\"][^'\"]*['\"] ?/?>)[^<]*</a>", src).map(completeImage),
                        pageCount,
                    };
                },
            },
