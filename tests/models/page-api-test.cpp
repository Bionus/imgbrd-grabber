...
void PageApiTest::testParseUrlBasic()
{
	...
	Page page(m_site, m_sites, tags);
	PageApi pageApi(&page, m_site, m_site->getApis().first(), tags);
...
void PageApiTest::testParseUrlLogin()
{
	...
	QStringList tags = QStringList() << "test" << "tag";
	Page page(site, m_sites, tags);
	PageApi pageApi(&page, site, site->getApis().first(), tags);
...
void PageApiTest::testParseUrlAltPage()
{
	QStringList tags = QStringList() << "test" << "tag";
	Site *site = m_sites.first();
	Page prevPage(site, m_sites, tags, 1000);
	Page page(site, m_sites, tags, 1001);
