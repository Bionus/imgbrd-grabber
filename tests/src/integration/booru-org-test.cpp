...
void BooruOrgTest::testHtml()
{
	for (int i = 0; i < 2; ++i)
	{
		QList<Image*> images = getImages("Gelbooru (0.1)", "rm.booru.org", "regex", "rating:safe", "results.html", i != 0);
...
void BooruOrgTest::testPageTags()
{
	for (int i = 0; i < 2; ++i)
	{
		QList<Tag> tags = getPageTags("Gelbooru (0.1)", "rm.booru.org", "regex", "rating:safe", "results.html", i != 0);
