...
void SankakuTest::testHtml()
{
	for (int i = 0; i < 2; ++i)
	{
		QList<Image*> images = getImages("Sankaku", "idol.sankakucomplex.com", "regex", "rating:safe", "results.html", i != 0);
...
void SankakuTest::testJson()
{
	for (int i = 0; i < 2; ++i)
	{
		QList<Image*> images = getImages("Sankaku", "idol.sankakucomplex.com", "json", "rating:safe", "results.json", i != 0);
...
void SankakuTest::testAnimatedUrls()
{
	for (int i = 0; i < 2; ++i)
	{
		QList<Image*> images = getImages("Sankaku", "idol.sankakucomplex.com", "regex", "animated rating:safe", "results-animated.html", i != 0);
